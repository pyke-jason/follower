  ...
  ws.on('message', (raw) => {
    try {
      ...
      switch (event.type) {
        ...
        case 'error':
          if (event.message?.includes('Persistent reconnect failure')) {
            sendSystemAlert({
              severity: 'critical',
              title: 'IBKR persistent reconnect failure',
              message: `Sidecar cannot reconnect to IB Gateway: ${event.message}. Possible competing session or Gateway down.`,
            });
          }
