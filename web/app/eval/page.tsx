...
export default function EvalPage() {
  ...
  const { data, isLoading } = useQuery<EvalResponse>({
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {data.totalLabeled.toLocaleString()} messages labeled | {data.totalWithSignals.toLocaleString()} trades | {data.totalSkipLabels.toLocaleString()} skips
          </p>
        </div>
        <Link
          to="/eval/review"
      ...
      <CardContent className="pt-4 pb-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1 rounded-lg border px-4 py-3">
            ...
            <span className="text-xl font-semibold">{reviewed.toLocaleString()} <span className="text-sm text-muted-foreground">/ {vs.total.toLocaleString()}</span></span>
          </div>
          <div className="flex flex-col gap-1 rounded-lg border px-4 py-3">
            <span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Unreviewed</span>
            <span className="text-xl font-semibold">{vs.unreviewed.toLocaleString()}</span>
          </div>
          <div className="flex flex-col gap-1 rounded-lg border bg-emerald-500/5 px-4 py-3">
            ...
            <span className="text-xl font-semibold text-emerald-700 dark:text-emerald-400">{vs.parserRight.toLocaleString()} <span className="text-sm">({pct(vs.parserRight)})</span></span>
          </div>
          <div className="flex flex-col gap-1 rounded-lg border bg-amber-500/5 px-4 py-3">
            ...
            <span className="text-xl font-semibold text-amber-700 dark:text-amber-400">{vs.labelRight.toLocaleString()} <span className="text-sm">({pct(vs.labelRight)})</span></span>
          </div>
        </div>
      </CardContent>
    </Card>
  ...
  return (
    <Card>
      ...
      <CardContent className="pt-3 pb-2 px-3">
        {sorted.length === 0 ? (
          <p className="text-sm text-muted-foreground py-2">No mismatches</p>
        ) : (
    ...
    <div>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t">
        <span className="text-xs text-muted-foreground">
          {total.toLocaleString()} total discrepancies
        </span>
        <div className="flex gap-2">
          <button
            onClick={() => onPageChange(Math.max(0, page - 1))}
            disabled={page === 0}
            className="px-3 py-1 text-xs rounded border disabled:opacity-30 hover:bg-muted"
          >
            Prev
          </button>
