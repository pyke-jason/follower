...
export function ChannelScopeSelector() {
  ...
  const [runtimeChannels, setRuntimeChannels] = useState<RuntimeChannelItem[]>([]);
  const [backtestChannels, setBacktestChannels] = useState<BacktestChannelItem[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api<ChannelsResponse>('/channels')
      .then((data) => {
        setRuntimeChannels(data.runtimeChannels);
        setBacktestChannels(data.backtestChannels);
  ...
  }, [open, fetched, setDefaultChannelId]);
